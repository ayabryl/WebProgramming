...
export default function ProductDetails(props) {
  const [chosenColor, setChosenColor] = useState(
    props.makeUpProduct.product_colors.legnth
      ? props.makeUpProduct.product_colors[0]
      : ""
  );

  const [amount, setAmount] = useState(
    props.makeUpProduct.amount ? props.makeUpProduct.amount : 1
  );
  ...
  const productsColors = props.makeUpProduct.product_colors.map((p) => (
    <Grid item onClick={() => setColor(p)} key={Math.random().toString()}>
      <span
        style={{
          height: "25px",
          width: "25px",
          borderColor:
            p.hex_value === chosenColor.hex_value ? "#000000" : p.hex_value,
          borderStyle: "solid",
          borderWidth: "0.2em",
        ...
        <Grid item>
          ...
          >
            ...
            <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
              ...
              <Select value={amount} label="amount" onChange={handleChange}>
